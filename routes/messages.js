...
router.get('/:id', function (req, res, next) {
    ...
    Models.Message.findOne({
        where: {id: messageId}, include: {model: Models.Reply, include: {model: Models.User}
        }
    })
            .then(function (data) {
                res.send(data)
...
router.post('/:id/reply', authentication, function (req, res, next) {
    ...
    var messageId = req.params.id;
    // ...tämä vastaus (Vinkki: lisää ensin replyToAdd-objektiin kenttä MessageId, 
    // jonka arvo on messageId-muuttujan arvo ja käytä sen jälkeen create-funktiota)
    ...
    Models.Reply.create(replyToAdd).then(function (data) {
        res.send(data);
    });
